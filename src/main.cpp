...
float satisfiability(string expression, string& values) {
    for(unsigned long int i = 0; i < values.size(); i++){
        if(values[i] == 'a'){
            string case0 = values;
            case0[i] = '0';
            float resultCase0 = satisfiability(expression, case0);

            string case1 = values;
            case1[i] = '1';
            float resultCase1 = satisfiability(expression, case1);
...
int main(int argc, char *argv[]){
    ...
    switch (argv[1][1]) {
        ...
        case 's':
            ...
            result = satisfiability(expression, values);
